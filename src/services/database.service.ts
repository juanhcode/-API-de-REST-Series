...
export const collections: { series?: mongoDB.Collection<Serie> } = {};
...
export async function connectToDatabase() {
    ...
    const gamesCollection = db.collection<Serie>(process.env.SERIES_COLLECTION_NAME);

    // Persist the connection to the Games collection
    collections.series = gamesCollection;
...
async function applySchemaValidation(db: mongoDB.Db) {
    const jsonSchema = {
        $jsonSchema: {
            bsonType: "object",
            required: ["name", "description", "season","gender"],
   ...
   await db.command({
        collMod: process.env.SERIES_COLLECTION_NAME,
        validator: jsonSchema
    }).catch(async (error: mongoDB.MongoServerError) => {
        if (error.codeName === 'NamespaceNotFound') {
            await db.createCollection(process.env.SERIES_COLLECTION_NAME, {validator: jsonSchema});
        }
    });
}
